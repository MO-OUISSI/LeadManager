...
const AgentCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Agent créé avec succès.' });
      navigate('/user/agents');
    } catch (error) {
      setStatus({ type: 'error', message: error.message || 'Impossible de créer cet agent.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="agent-form-page">
          ...
          <h2 className="page-title">Créer un nouvel Agent</h2>
                  <button onClick={() => navigate('/user/agents')} className="btn btn-secondary">
                    <ArrowLeft size={18} />
                    <span>Retour</span>
                  </button>
                </div>
        ...
        <form onSubmit={handleSubmit} className="agent-form">
          ...
          <div className="form-actions">
            <button type="button" onClick={() => navigate('/user/agents')} className="btn btn-secondary">
              Annuler
            </button>
            <button type="submit" className="btn btn-primary" disabled={saving}>
              <Save size={18} />
              <span>{saving ? 'Création...' : "Créer l'Agent"}</span>
            </button>
          </div>
        </form>
      </div>
